...
export default function Sidebar({ page, setPage, owner, setOwner,
  ...
  const WILSONS_ONLY_PAGES = new Set(['history', 'blueprint']);
      ...
      <nav style={{ flex: 1, padding: '0.5rem 0' }}>
        {navItems.map(item => (
          ...
          >
            ...
            {!isWilsonsLeague && WILSONS_ONLY_PAGES.has(item.id) && (
              <span style={{ fontSize: '10px', opacity: 0.45, marginLeft: '4px', flexShrink: 0 }}>🔒</span>
            )}
          </button>
        ))}
      </nav>
