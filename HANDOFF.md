...
5. **Trade Value Trajectory** — line chart of roster KTC value over time. Requires snapshot storage strategy. Add to Home or Blueprint.
   - Note for whoever picks this up: `leagueRosters.json` only ever holds the *current* snapshot (flat list, no history) — there's no existing per-date archive to chart against, so this still needs its own storage plan (e.g. append a dated snapshot file each notebook run, or a new Firestore collection) before any charting work can start.
