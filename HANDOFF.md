...
3. **Prime Windows Chart** — ✅ Done. `PrimeWindowsChart` in `src/pages/TeamDeepDive.js`, placed between Dynasty Matrix and Positional Grades vs League. Client-side only (no notebook changes) — shows top 12 roster players by KTC value as horizontal timeline bars across ages 21–36, colored by phase (Rising orange/Prime green/Declining red) using the same per-position age cutoffs as Dynasty Matrix, plus a per-position max-age cutoff (QB 36, RB 30, WR/TE 33) beyond which the bar is left blank. White vertical tick marks each player's current age. Wilson's only — not added to CLTC.
